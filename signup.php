...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //  Mengatasi BUFFER OVERFLOW untuk membuat akun baru
    if (strlen($user) > 20 || strlen($pass) > 20) {
        $message = "<div class='msg-error'>Input terlalu panjang! (Maks 20 karakter).</div>";
    } else {
        // Hash Password
        $hashed_password = password_hash($pass, PASSWORD_BCRYPT);

        // Mitigasi SQL Injection (Prepared Statement)
        ...
        try {
            ...
            if ($e->getCode() == 1062) {
                $message = "<div class='msg-error'>Gagal mendaftar. Username '<strong>" . htmlspecialchars($user) . "</strong>' sudah terpakai!</div>";
            } else {
...
<head>
    ...
    <style>
        ...
        .checkbox-container { text-align: left; margin-top: -10px; margin-bottom: 20px; font-size: 14px; display: flex; align-items: center; gap: 5px; }
        .checkbox-container input { width: auto; margin: 0; }
        .checkbox-container label { cursor: pointer; user-select: none; color: #333; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <form method="POST">
            ...
            <input type="password" name="password" id="signupPassword" placeholder="Buat password yang kuat" required>
            
            <div class="checkbox-container">
                <input type="checkbox" id="showSignup" onclick="togglePassword('signupPassword')">
                <label for="showSignup">Tampilkan Password</label>
            </div>
