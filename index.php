...
// --- LOGIKA 1: PROSES LOGIN (AMAN) ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login_submit'])) {
    $user = $_POST['username'];
    $pass = $_POST['password'];

    // Perlindungan BUFFER OVERFLOW pada laman login
    if (strlen($user) > 20 || strlen($pass) > 20) {
        $error_login = "Input terlalu panjang! (Maks 20 karakter)";
    } else {
...
// --- LOGIKA 2: PROSES INPUT KOMENTAR (AMAN) ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['comment_submit']) && isset($_SESSION['user_id'])) {
    $comment = $_POST['comment'];
    
    //  Mitigasi Buffer Overflow untuk kolom komentar 
    if(strlen($comment) > 200) {
        $error_comment = "Komentar terlalu panjang! (Maks 200 karakter)";
    } else {
...
<head>
    ...
    <style>
        ...
        .checkbox-container { margin-top: -5px; margin-bottom: 15px; text-align: left; display: flex; align-items: center; gap: 5px; }
        .checkbox-container input { width: auto; margin: 0; }
        .checkbox-container label { font-size: 14px; color: #65676b; cursor: pointer; user-select: none; }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <?php if(!isset($_SESSION['user_id'])): ?>
        <div class="card">
            ...
            <form method="POST">
                ...
                <input type="password" name="password" id="indexPassword" placeholder="Masukkan password" required>
                
                <div class="checkbox-container">
                    <input type="checkbox" id="showIndexPass" onclick="togglePassword('indexPassword')">
                    <label for="showIndexPass">Tampilkan Password</label>
                </div>
    ...
    <?php else: ?>
        <div class="card">
            ...
            <?php if($error_comment) echo "<div class='error-msg'>$error_comment</div>"; ?>
            ...
            <table class="admin-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><div class="hash-text"><?= $u['password_hash'] ?></div></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
        ...
        <?php if($result->num_rows > 0): ?>
            <?php while($row = $result->fetch_assoc()): ?>
                <div class="comment-item">
                    <span class="comment-user"><?= htmlspecialchars($row['username']) ?></span>
                    <span class="comment-date"><?= $row['created_at'] ?></span>
                    <div class="comment-text"><?= nl2br(htmlspecialchars($row['comment'])) ?></div>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p style="color: #999; text-align: center; padding: 20px;">Belum ada diskusi.</p>
        <?php endif; ?>
    </div>
